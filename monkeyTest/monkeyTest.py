...
def doMonkeyTest(times,actionCount,outdir):
    ...
    while count <= times:
        outfileFd.write("-"*100)
        outfileFd.write("\n")
        outfileFd.write("Do " + str(count) + "TH at:" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        res = subprocess.Popen('adb shell monkey -v-v-v ' + str(actionCount), shell=True,
...
countAction=100
outfile='.'
opts,args = getopt.getopt(sys.argv[1:],'-h:-t:-c:-o:-v',['help','times','count-action','outdir','version'])
